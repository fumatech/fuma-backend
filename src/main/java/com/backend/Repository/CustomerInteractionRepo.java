package com.backend.Repository;

import java.util.List;

import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.stereotype.Repository;

import com.backend.Entity.CustomerInteraction;

@Repository
public interface CustomerInteractionRepo extends JpaRepository<CustomerInteraction, Long> {

    List<CustomerInteraction> findByLeadIdOrderByInteractionDateDesc(Long leadId);

    List<CustomerInteraction> findByCustomerIdOrderByInteractionDateDesc(Long customerId);

    List<CustomerInteraction> findBySalespersonIdOrderByInteractionDateDesc(Long salespersonId);
}
